...
2. **向兄弟借键**：若删除后节点空且相邻兄弟有两个键，则从兄弟借一个，通过父节点做键的上移和下移来恢复平衡。

3. **合并节点**：若兄弟也只有一个键（无法借），就把父节点中间的分隔键下移，与兄弟合并成一个新节点，同时父节点减少一个键。

4. **内部节点删除**：若要删除的键在内部节点中，就先用右子树的最小键（或左子树的最大键）替换它，把问题转化为叶节点删除。

5. **递归修复与根收缩**：若合并导致父节点也空，就向上递归执行同样的修复；若最终根也空，则提升唯一子节点为新根，使树高度减一。


### **B+tree**
---
每个节点又有相对应的记录，索引只充当索引的功能。  

**insertion**  
---
当页未满时：直接插入。

当叶子页满时：分裂叶子页，把中间键提到上层索引。

当连上层索引也满时：连锁分裂（cascade split），最终可能导致树高度增加（新根生成）。  
...
如果索引页也太空，就继续向上合并，直到平衡或收缩为新根。

## **Hash manbo ~~** wow

**双重哈希（Double Hashing）** 是一种开放定址策略，使用两个哈希函数：
第一个决定起始位置，第二个决定每次跳跃步长。
这样可以避免线性或二次探测的聚集问题，使冲突分布更随机、更均匀。**（谁知道为啥没有拉链法）**

## Sorting！ Big Time！ True Algorithm！

### 📘 内部排序算法综合比较表

| 排序算法                        | 最坏时间复杂度    | 平均时间复杂度    | 最好时间复杂度    | 辅助空间复杂度  | 稳定性   |
| --------------------------- | ---------- | ---------- | ---------- | -------- | ----- |
| **直接插入排序 (Insertion Sort)** | Θ(n²)      | Θ(n²)      | Θ(n)       | Θ(1)     | ✅ 稳定  |
| **冒泡排序 (Bubble Sort)**      | Θ(n²)      | Θ(n²)      | Θ(n)       | Θ(1)     | ✅ 稳定  |
| **选择排序 (Selection Sort)**   | Θ(n²)      | Θ(n²)      | Θ(n²)      | Θ(1)     | ❌ 不稳定 |
| **希尔排序 (Shell Sort)**       | Θ(n^1.5)   | Θ(n^1.5)   | Θ(n^1.5)   | Θ(1)     | ❌ 不稳定 |
| **快速排序 (Quick Sort)**       | Θ(n²)      | Θ(n log n) | Θ(n log n) | Θ(log n) | ❌ 不稳定 |
| **归并排序 (Merge Sort)**       | Θ(n log n) | Θ(n log n) | Θ(n log n) | Θ(n)     | ✅ 稳定  |
| **堆排序 (Heap Sort)**         | Θ(n log n) | Θ(n log n) | Θ(n log n) | Θ(1)     | ❌ 不稳定 |
| **基数排序 (Radix Sort)**       | Θ(d·(n+r)) | Θ(d·(n+r)) | Θ(d·(n+r)) | Θ(n+r)   | ✅ 稳定  |

---

### 八种排序算法的核心思想

1. **直接插入排序**：依次将元素插入到前面已排序序列的合适位置。
2. **冒泡排序**：不断比较相邻元素，较大的逐步“冒泡”到右端。
3. **选择排序**：每一轮选择最小元素放到未排序序列的开头。
4. **希尔排序**：按步长分组做插入排序，逐步缩小步长直到整体有序。
5. **快速排序**：选择一个枢轴，将序列划分为左右两部分递归排序。
6. **归并排序**：分治策略，将序列不断二分并在回溯时合并有序子列。
7. **堆排序**：利用堆结构反复取最大（或最小）元素调整堆结构。
8. **基数排序**：按数字（或字符）各位从低到高依次分配和收集。
